...
async def chat(
    ...
    if (
        ...
        await manager.append(Message("system", workspace_prompt, hide=True, quiet=True))
    ...
    while True:
            ...
            while prompt_msgs:
                ...
                while True:
                    try:
                        set_interruptible()
                        response_msgs = []
                        async for msg in step(manager.log, stream, confirm_func):
                            if msg.content.strip():  # Only append non-empty messages
                                response_msgs.append(msg)
                    except KeyboardInterrupt:
                        console.log("Interrupted. Stopping current execution.")
                        await manager.append(Message("system", "Interrupted"))
                        break
                    finally:
                        clear_interruptible()

                    for response_msg in response_msgs:
                        await manager.append(response_msg)
                        # run any user-commands, if msg is from user
                        if response_msg.role == "user" and await execute_cmd(
        ...
        async for msg in step(manager.log, stream, confirm_func):  # pragma: no cover
            await manager.append(msg)
            # run any user-commands, if msg is from user
            if msg.role == "user" and await execute_cmd(msg, manager, confirm_func):
                break
...
async def step(
    ...
    try:
        ...
        # Get memory manager if available (log might be a LogManager instance)
        memory_manager = None
        if isinstance(log, LogManager) and hasattr(log, "memory_manager"):
            memory_manager = log.memory_manager
            
            # Log memory status for debugging
            if memory_manager and memory_manager.mode != MemoryMode.DISABLED:
                logger.info(f"Memory mode: {memory_manager.mode}")
                logger.info(f"Memory failures: {memory_manager._failure_count}")
        ...
        if memory_manager and memory_manager.mode != MemoryMode.DISABLED:
            try:
                ...
                memory_context = await memory_manager.get_relevant_context(
                    log.messages[-1].content if log.messages else ""
                )
                if memory_context:
                    logger.info(f"Found {len(memory_context)} relevant messages in memory")
                    msgs.extend(memory_context)
            except Exception as e:
                logger.warning(f"Failed to get memory context: {e}")
        
        # Add current messages
        msgs.extend(log.messages)

        # Log message count for debugging
        logger.debug(f"Total messages for model: {len(msgs)}")
        for m in msgs:
            logger.debug(f"Message: {m.role} - {m.content[:50]}...")
        ...
        if msg_response:
            ...
            for reply_msg in execute_msg(msg_response, confirm):
                yield reply_msg
    except KeyboardInterrupt:
        clear_interruptible()
